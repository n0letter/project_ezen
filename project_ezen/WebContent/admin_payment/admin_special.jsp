  ...
  <div class="payspecial_wrap">
    ...
    <div class="payspecial_table"> 	        
      <table class="payspecial_table_wrap" id="special_content">
       ...
       <% int num=1; %> 
	    <c:forEach var="dto" begin="0" step="1" items="${ adminspay_list }"> 
          <tr>
            <td><%= num %><% num=num+1; %></td> 
    ...
    <ul class="pagination justify-content-center">
	  <c:if test="${paycount > 0}">
		...
		<c:if test="${nowPageGroup > 1}">
		  <li class="page-item">
			<a class="page-link" href="<c:url value = "/project_ezen/admin_payment/admin_special.do?page=${(nowPageGroup-2)*pageGroupSize+1 }&pageSize=${pageSize}"/>">
			  이전
			</a>
	      </li>
        </c:if>
		  ...
		  <c:choose>
	        <c:when test="${param.page eq i }">
			  <li class="page-item active" aria-current="page">
				<a class="page-link" href="<c:url value="/project_ezen/admin_payment/admin_special.do?page=${i}&pageSize=${pageSize}"/>">				     
				  ${i} 
				</a>
			  </li>
			</c:when>
        ...
        <c:if test="${nowPageGroup < pageGroupCount}">
		  <li class="page-item">
		    <a class="page-link" href="<c:url value = "/project_ezen/admin_payment/admin_special.do?page=${nowPageGroup*pageGroupSize+1}&pageSize=${pageSize}"/>">
		      다음
		    </a>
	      </li>
	    </c:if>
